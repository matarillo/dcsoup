...
namespace NSoup.Parser
{
	/// <summary>A character queue with parsing helpers.</summary>
	/// <remarks>A character queue with parsing helpers.</remarks>
	...
	public class TokenQueue
	{
		...
		/// <remarks>Create a new TokenQueue.</remarks>
		...
		/// <remarks>Retrieves but does not remove the first character from the queue.</remarks>
		/// <returns>First character, or 0 if empty.</returns>
		public virtual char Peek()
		{
			return IsEmpty() ? 0 : queue[pos];
		}

		/// <summary>Add a character to the start of the queue (will be the next character retrieved).
		/// 	</summary>
		/// <remarks>Add a character to the start of the queue (will be the next character retrieved).
		/// 	</remarks>
		...
		/// <remarks>Add a string to the start of the queue.</remarks>
		/// <param name="seq">string to add.</param>
		public virtual void AddFirst(string seq)
		{
			// not very performant, but an edge case
			queue = seq + Sharpen.Runtime.Substring(queue, pos);
			pos = 0;
		}

		/// <summary>Tests if the next characters on the queue match the sequence.</summary>
		/// <remarks>Tests if the next characters on the queue match the sequence. Case insensitive.
		/// 	</remarks>
		...
		/// <remarks>Case sensitive match test.</remarks>
		...
		/// <remarks>Tests if the next characters match any of the sequences. Case insensitive.
		/// 	</remarks>
		...
		public virtual bool MatchesStartTag()
		{
			...
			return (RemainingLength() >= 2 && queue[pos] == '<' && char.IsLetter(queue[pos + 
				1]));
		}
		...
		/// <remarks>Tests if queue starts with a whitespace character.</remarks>
		...
		/// <remarks>Test if the queue matches a word character (letter or digit).</remarks>
		...
		/// <remarks>Drops the next character off the queue.</remarks>
		...
		/// <remarks>Consume one character off queue.</remarks>
		...
		/// 	</summary>
		/// <remarks>Pulls a string off the queue, up to but exclusive of the match sequence, or to the queue running out.
		/// 	</remarks>
		...
		public virtual string ConsumeTo(string seq)
		{
			...
			if (offset != -1)
			{
				string consumed = Sharpen.Runtime.Substring(queue, pos, offset);
				pos += consumed.Length;
				return consumed;
			}
			else
		...
		{
			int start = pos;
			string first = Sharpen.Runtime.Substring(seq, 0, 1);
			...
			string data = Sharpen.Runtime.Substring(queue, start, pos);
			return data;
		}
		...
		/// 	</remarks>
		/// <param name="seq">any number of terminators to consume to. <b>Case insensitive.</b>
		/// 	</param>
		...
		public virtual string ChompBalanced(char open, char close)
		{
			...
			while (depth > 0);
			return (end >= 0) ? Sharpen.Runtime.Substring(queue, start, end) : string.Empty;
		}

		/// <summary>Unescaped a \ escaped string.</summary>
		/// <remarks>Unescaped a \ escaped string.</remarks>
		...
		/// <remarks>Pulls the next run of whitespace characters of the queue.</remarks>
		...
		/// <remarks>Retrieves the next run of word type (letter or digit) off the queue.</remarks>
		...
		public virtual string ConsumeWord()
		{
			...
			return Sharpen.Runtime.Substring(queue, start, pos);
		}
		...
		/// 	</summary>
		/// <remarks>Consume a CSS element selector (tag name, but | instead of : for namespaces, to not conflict with :pseudo selects).
		/// 	</remarks>
		/// <returns>tag name</returns>
		...
		/// <remarks>Consume and return whatever is left on the queue.</remarks>
		/// <returns>remained of queue.</returns>
		public virtual string Remainder()
		{
			string remainder = Sharpen.Runtime.Substring(queue, pos, queue.Length);
			pos = queue.Length;
			return remainder;
		}
		...
		{
			return Sharpen.Runtime.Substring(queue, pos);
		}
	}
}
